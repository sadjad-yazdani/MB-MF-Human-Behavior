...
function [Train,Test,AllData]=GetKFoldRegressorDataFromRepository(KF,AddFullyAgent)
if nargin<2
    AddFullyAgent=false;
end
...
TSTN = round(AllData.SNum/KF);
TRNN = AllData.SNum - TSTN;
...
for k=1:KF
    ...
    Test.Features(:,:,k)       = AllData.Features(TestIndex,:);
    Test.Label(:,k)            = AllData.Label(TestIndex);
    Test.FittedW(:,:,k)        = AllData.FittedW(TestIndex,:);
    
    Train.Features(1:TRNN,:,k)      = AllData.Features(TrainIndex,:);
    Train.Label(1:TRNN,k)           = AllData.Label(TrainIndex);
    Train.FittedW(1:TRNN,:,k)       = AllData.FittedW(TrainIndex,:);
end
